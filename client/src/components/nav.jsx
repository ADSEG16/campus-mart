...
const CampusNavbar = () => {
  ...
  const refreshNotificationBadge = React.useCallback(async () => {
    const token = getStoredAuthToken();
    if (!token) {
    ...
    try {
      ...
      // Keep previous badge state when notifications fail to load.
    }
  }, [currentUserId, readMarkerKey]);
  ...
  return (
    ...
    >
      ...
      {/* Desktop Links */}
      {isAdmin ? renderAdminMenu() : isVerifiedUser ? renderVerifiedMenu() : renderPublicMenu()}
            ...
            <div className="space-y-2">
              {!isMenuReady ? (
                ...
                <>
                  ...
                  <NavLink to="/admin" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-3 rounded-xl hover:bg-gray-50 text-sm font-medium text-gray-700">
                    Admin
                  </NavLink>
                </>
              ) : isVerifiedUser ? (
                ...
                <>
                  ...
                  <Link to="/signup" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-3 rounded-xl hover:bg-gray-50 text-sm font-medium text-gray-700">
                    Sign Up
                  </Link>
                  <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="block text-center px-4 py-3 rounded-xl bg-blue-600 text-white text-sm font-medium mt-4">
                    Log In
                  </Link>
                </>
              )}
